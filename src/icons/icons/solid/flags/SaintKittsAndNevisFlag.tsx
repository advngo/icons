import * as React from "react";
import { SVGProps } from "react";

const SvgSaintKittsAndNevisFlag = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512.001 512.001"
    xmlSpace="preserve"
    width={24}
    height={24}
    {...props}
  >
    <path
      style={{
        fill: "#73af00",
      }}
      d="M473.655 423.724H38.345C17.167 423.724 0 406.557 0 385.379V126.621c0-21.177 17.167-38.345 38.345-38.345h435.31c21.177 0 38.345 17.167 38.345 38.345V385.38c0 21.177-17.167 38.344-38.345 38.344z"
    />
    <path
      style={{
        fill: "#ffe15a",
      }}
      d="M0 334.022v26.384l415.356-272.13h-40.271zM136.914 423.724 512 177.977v-26.383L96.644 423.724z"
    />
    <path
      style={{
        fill: "#464655",
      }}
      d="M473.655 88.276h-58.3L0 360.406v24.973c0 21.177 17.167 38.345 38.345 38.345h58.3L512 151.594v-24.973c0-21.178-17.167-38.345-38.345-38.345z"
    />
    <path
      style={{
        fill: "#f5f5f5",
      }}
      d="m156.019 275.898 18.119 14.59 19.67-12.419c2.547-1.608 5.675.951 4.602 3.766l-8.277 21.74 17.89 14.869c2.316 1.926.849 5.691-2.159 5.542l-23.234-1.154-8.614 21.61c-1.116 2.797-5.15 2.567-5.938-.341l-6.082-22.454-23.213-1.514c-3.006-.196-4.032-4.105-1.511-5.752l19.475-12.724-5.734-22.546c-.741-2.917 2.66-5.101 5.006-3.213zM323.007 166.29l18.119 14.59 19.67-12.42c2.547-1.608 5.675.951 4.602 3.766l-8.277 21.74 17.89 14.869c2.316 1.926.85 5.69-2.159 5.542l-23.234-1.154-8.613 21.61c-1.116 2.797-5.15 2.566-5.938-.341l-6.082-22.454-23.213-1.514c-3.006-.196-4.032-4.105-1.511-5.752l19.475-12.724-5.734-22.546c-.741-2.918 2.659-5.101 5.005-3.212z"
    />
    <path
      style={{
        fill: "#ff4b55",
      }}
      d="M512 177.977 136.914 423.724h336.742c21.177 0 38.345-17.167 38.345-38.345L512 177.977z"
    />
  </svg>
);

export default SvgSaintKittsAndNevisFlag;
