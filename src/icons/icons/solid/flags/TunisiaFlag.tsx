import * as React from "react";
import { SVGProps } from "react";

const SvgTunisiaFlag = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512 512"
    xmlSpace="preserve"
    width={24}
    height={24}
    {...props}
  >
    <path
      style={{
        fill: "#ff4b55",
      }}
      d="M473.655 423.724H38.345C17.167 423.724 0 406.557 0 385.379V126.621c0-21.177 17.167-38.345 38.345-38.345h435.31c21.177 0 38.345 17.167 38.345 38.345V385.38c0 21.177-17.167 38.344-38.345 38.344z"
    />
    <path
      style={{
        fill: "#f5f5f5",
      }}
      d="M257.002 167.724c48.718 0 88.205 39.594 88.205 88.276 0 48.823-39.488 88.276-88.205 88.276-48.822 0-88.346-39.453-88.346-88.276-.001-48.682 39.523-88.276 88.346-88.276"
    />
    <path
      style={{
        fill: "#ff4b55",
      }}
      d="M273.042 305.466c-27.327 0-49.493-22.165-49.493-49.461 0-27.23 22.165-49.395 49.493-49.395a48.86 48.86 0 0 1 25.405 7.102c1.112.674 2.282-.782 1.355-1.695-11.138-10.974-26.389-17.771-43.236-17.771-34.2 0-61.891 27.692-61.891 61.761 0 34.135 27.691 61.825 61.891 61.825 16.838 0 32.083-6.792 43.219-17.763.927-.913-.243-2.369-1.355-1.696a48.838 48.838 0 0 1-25.388 7.093z"
    />
    <path
      style={{
        fill: "#ff4b55",
      }}
      d="m268.52 221.387 13.644 18.526 21.945-6.912c2.841-.895 5.186 2.342 3.452 4.762l-13.402 18.701 13.355 18.735c1.729 2.425-.625 5.656-3.464 4.754l-21.927-6.968-13.692 18.49c-1.772 2.393-5.572 1.154-5.591-1.825l-.149-23.007-21.816-7.307c-2.825-.946-2.819-4.943.008-5.881l21.835-7.252.207-23.007c.026-2.978 3.828-4.208 5.595-1.809z"
    />
  </svg>
);

export default SvgTunisiaFlag;
